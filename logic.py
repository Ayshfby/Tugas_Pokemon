...
class Pokemon:
    ...
    async def info(self):
        ...
        return f"The name of your Pokémon: {self.name}\nhp: {self.hp}\npower: {self.power}\nindex: {self.index}"  # Returning the string with the Pokémon's name
    ...
    async def get_index(self):
        ...
        async with aiohttp.ClientSession() as session:  # Opening an HTTP session
            async with session.get(url) as response:  # Sending a GET request
                if response.status == 200:
                    data = await response.json()  # Receiving and decoding JSON response
                    if data["game_indices"]:   # cek dulu list-nya ada isinya
...
class Wizard(Pokemon):
    ...
    async def info(self):
        # A method that returns information about the pokémon
        return f"pokemon kamu adalah penyihir" + await super().info()
    
    async def feed(self, feed_interval = 0.5, hp_increase = 10 ):
        current_time = datetime.now()  
        delta_time = timedelta(hours=feed_interval)  
        if (current_time - self.last_feed_time) > delta_time:
            self.hp += hp_increase
...
class Fighter(Pokemon):
    ...
    async def info(self):
        # A method that returns information about the pokémon
        return f"pokemon kamu adalah petarung" + await super().info()
    
    async def feed(self, feed_interval = 5, hp_increase = 20 ):
        current_time = datetime.now()  
        delta_time = timedelta(hours=feed_interval)  
        if (current_time - self.last_feed_time) > delta_time:
            self.hp += hp_increase
